...
class NotificationsWorker {
    ...
    private function sendEmail($contents, $recipientInfo, $messageData, $serverName, $user_settings = array(), $account_info = array()) {
        try {
            //set a default template to start with
            $template_name = TrackingHelper::EMAILTYPE_NOTIFICATIONS;
                ...
                if ($use_new_replies)
                {
                    ...
                    if ($messageData['type'] == 'text'){
                      ...
                      //TODO if ab test is successful, these checks can be moved to a helper method
                        ...
                        $email_stream_ab_test = AbHandler::idInTest($recipientInfo['user_id'], 'email_ui_stream_note');
                        $email_stream_gatekeeper = AbHandler::idInTest($recipientInfo['district_id'], 'email_ui_stream_gk ');
                        if ($email_stream_ab_test != AbHandler::CONTROL	|| $email_stream_gatekeeper != AbHandler::CONTROL ){
                            $template_name = TrackingHelper::EMAILTYPE_NOTIFICATIONS_5_NOTE;
                        }
                        else{
                            $template_name = TrackingHelper::EMAILTYPE_NOTIFICATIONS_4;
                        }
                        ...
                        //get a count on the number of attached links/files and prep some rendering
                        $attached_links = makeArray($messageData['links']);
                        $count_attachments= count($attached_links) + count($messageData['files']);
                        $additional_attachment_html = '';

                        if ($count_attachments > 1){
                            $attachment_text = '<a href="'.$messageLink['formattedLink'].'" style="text-decoration:none; font-family: Helvetica, Arial, sans-serif; font-size: 14px; color:rgb(136, 141, 141);"><b>' .$count_attachments . '</b> attachments</a>';
                        }
                        else if ($count_attachments == 1) {
                            $attachment_text = '<a href="'.$messageLink['formattedLink'].'" style="text-decoration:none; font-family: Helvetica, Arial, sans-serif; font-size: 14px; color:rgb(136, 141, 141);"><b>' .$count_attachments . '</b> attachment</a>';
                        }

                        if ($count_attachments > self::MAX_THUMBS_TO_SHOW){
                            $additional_attachments = $count_attachments - 2;
                            ...
                            $additional_attachment_html = '<tr>
                                                                <td style="padding:10px 0 0 20px">
                                                                    <a href="'.$messageLink['formattedLink'].'" style="font-family: Helvetica, Arial, sans-serif; font-weight: bold; font-size: 13px; color: rgb(17, 85, 204); text-decoration:none" class="additional">Show '.$additional_attachments . ' more ' .$word.'</a>
                                                                </td>
                                                            </tr>';
                        }
                        ...
                        //we have a url, let's try to render some thumbs
                        if (!empty($messageData['links'][$i]['url'])){
                            foreach ($messageData['links'] as $link){

                                //we render a maximum of 2 thumbs
                                if ($i < 2){
                                    //figure out the domain of the url for the email render
                                    ...
                                    $thumbs[$i] = $open_graph_info->image;
                                    //we couldn't get the open graph image, let's hope BE already processed and grab from there
                                    if (empty($open_graph_info->image)){
                                        //sleep and give BE some time to generate the preview link thumb
                                        sleep(30);
                                        $link_info = Links::getInstance()->getMessagesLinks($messageData['message_id']);
                                        $thumb_from_db = $link_info[$i]['thumb_url'];
                                        if (!empty($thumb_from_db)){
                                            $thumbs[$i] = THUMBS_SERVER.$thumb_from_db;
                                        }
                                        else{
                                            //set a default image if we still don't have one. Worse case scenario. oh well
                                            $thumbs[$i] = PIC_SERVER.'default_155_2.jpg';
                                        }
                                    }
                                    ($i + 1 == $count_attachments || $i + 1 ==  self::MAX_THUMBS_TO_SHOW) ? $bottom_border = '1px solid rgb(194, 198, 197)' : $bottom_border = '0px';
                                    ($i == 0) ? $top_border = '1px solid rgb(194, 198, 197)' : $top_border = '0px';

                                    $attachment_html[] = '<table width="540" cellpadding="0" cellspacing="0" border="0" align="center" style="border-top: '.$top_border.'; border-bottom: '.$bottom_border.'" class="devicewidthinner">
                                                                  <tbody>
                                                                     <tr>
                                                                        <td valign="middle" width="82" style="padding: 0px 20px 0px 20px;" class="link_tmb">
                                                                        <br />
                                                                           <div class="imgpop">
                                                                              <a href="'.$link['url'].'"><img src="'.$thumbs[$i].'" alt="'.$link['desc'].'" border="0" style="border-radius:3px;display:block; border:none; outline:none; text-decoration:none;" st-image="edit" class="link_tmb" width="82" height="62"></a>
                                                                           </div>
                                                                           <br />
                                                                        </td>
                                                                        <td width="458" valign="middle" style="font-family: Helvetica, Arial, sans-serif;font-size: 13x; color: #2b2e38; padding: 0px 20px 0px 0px;" align="left" class="txtpad">

                                                                           <a href="'.$link['url'].'" style="text-align: left;font-family: Helvetica, Arial, sans-serif; font-size: 13px;line-height: 18px; font-weight:bold; text-decoration: none;" class="primaryLink">
                                                                            '.$link['desc'].'</a><br />
                                                                           <a style="color: rgb(184, 184, 184);text-decoration: none;" href="'.$domain.'" class="secondaryLink">'.$domain.'</a>
                        ...
                        if (!empty($messageData['files']) && $i < 2 && $template_name == TrackingHelper::EMAILTYPE_NOTIFICATIONS_5_NOTE){

                            $file_types_to_default_icons = array(
                                'docx' => PATH2_IMAGES . 'icons/large/doc.png',
                                'doc'  => PATH2_IMAGES . 'icons/large/doc.png',
                                'pdf'  => PATH2_IMAGES . 'icons/large/pdf.png',
                                'pptx' => PATH2_IMAGES . 'icons/large/ppt.png',
                                'ppt'  => PATH2_IMAGES . 'icons/large/ppt.png',
                                'xlsx' => PATH2_IMAGES . 'icons/large/xls.png',
                                'xls'  => PATH2_IMAGES . 'icons/large/xls.png',
                                'jpg'  => PATH2_IMAGES . 'icons/large/image.png',
                                'jpeg' => PATH2_IMAGES . 'icons/large/image.png',
                                'gif'  => PATH2_IMAGES . 'icons/large/image.png',
                                'png'  => PATH2_IMAGES . 'icons/large/image.png',
                                'mp3'  => PATH2_IMAGES . 'icons/large/media.png',
                                'avi'  => PATH2_IMAGES . 'icons/large/media.png',
                                'mpg'  => PATH2_IMAGES . 'icons/large/media.png',
                                'mpeg' => PATH2_IMAGES . 'icons/large/media.png',
                                'mov'  => PATH2_IMAGES . 'icons/large/media.png',
                            );

                            $files = ($messageData['files']);
                            foreach ($files as $file){
                                if ($i < 2){
                                    //get the extension and map it to an image
                                    $pathinfo = pathinfo($file['file_name']);
                                    $extension = $pathinfo['extension'];
                                    if (!empty($extension)){
                                        if (isset($file_types_to_default_icons[$extension])){
                                            $thumbs[$i] = $file_types_to_default_icons[$extension];
                                        }
                                    }
                                    else {
                                        $thumbs[$i] = PATH2_IMAGES . 'icons/large/media.png';
                                    }
                                    ($i + 1 == $count_attachments || $i + 1 ==  self::MAX_THUMBS_TO_SHOW) ? $bottom_border = '1px solid rgb(194, 198, 197)' : $bottom_border = '0px';
                                    ($i == 0) ? $top_border = '1px solid rgb(194, 198, 197)' : $top_border = '0px';
                                    $attachment_html[] = '<table width="540" cellpadding="0" cellspacing="0" border="0" align="center" class="devicewidthinner" style="border-top: '.$top_border.'; border-bottom: '.$bottom_border.'">
                                                                  <tbody>
                                                                     <tr>
                                                                        <td valign="middle" width="82" style="padding: 0px 20px 0px 20px;" class="link_tmb">
                                                                        <br />
                                                                           <div class="imgpop">
                                                                              <a href="'.$messageLink['formattedLink'].'"><img src="https:'.$thumbs[$i].'" alt="'.$file['file_name'].'" border="0" style="border-radius:3px;display:block; border:none; outline:none; text-decoration:none;" st-image="edit" class="link_tmb" width="82" height="62"></a>
                                                                           </div>
                                                                           <br />
                                                                        </td>
                                                                        <td width="458" valign="middle" style="font-family: Helvetica, Arial, sans-serif;font-size: 13px; color: #2b2e38;line-height: 18px; padding: 0px 20px 0px 0px;" align="left" class="txtpad">

                                                                           <a href="'.$messageLink['formattedLink'].'" style="text-align: left;font-family: Helvetica, Arial, sans-serif; font-size: 13px;line-height: 18px; font-weight:bold; text-decoration: none;" class="primaryLink">
                                                                            '.$pathinfo['filename'].'</a><br />
                                                                           <a style="color: rgb(184, 184, 184);" class="secondaryLink">'.strtoupper($extension).' File</a>
                        ...
                        $misc_data_tracking = array(TrackingHelper::MISCELLANEOUS_DATA_AB_TEST => array(TrackingHelper::MISCELLANEOUS_DATA_AB_TEST_NAME => 'email_ui_stream_note,  ', TrackingHelper::MISCELLANEOUS_DATA_AB_TEST_VARIANT => $email_stream_ab_test));
                    ...
                    if (!empty($contents['group_name'])){
                        $sent_to_html = '<span class="color:rgb(90, 91, 84);"> to </span><span style="color: rgb(23, 81, 163);"><a href="'.ENVIROMENT_DOMAIN.'/home#/group?id='.$contents['group_id'].'" style="text-decoration:none;" class="primaryLink">'.$contents['group_name'].'</a>:</span>';
                    }
                    else{
                        $sent_to_html = '<span class="color:rgb(90, 91, 84);">:</span>';
                    }
                    ...
                    $values2insert = array (
                        ...
                        '{LINKS_THUMB}'                 => isset($thumbs[0]) ? $thumbs[0] : null,
                        '{LINKS_HEIGHT}'                => isset($thumbs[0]) ? 62 : 0,
                        '{LINKS_TOP_SPACING}'           => isset($thumbs[0]) ? 20 : 0,
                        '{LINKS_BOTTOM_SPACING}'        => isset($thumbs[0]) ? 15 : 0,
                        '{LINKS_WIDTH}'                 => isset($thumbs[0]) ? 82 : 0,
                        ...
                        '{ATTACHMENT_HTML}'             => !empty($attachment_html) ? $attachment_html : null,
                        '{ADDITIONAL_ATTACHMENT_HTML}'  => isset($additional_attachment_html) ? $additional_attachment_html : null,
                        '{SEND_DATE}'                   => $currentDate,
                        '{SENT_TO_HTML}'                => $sent_to_html
